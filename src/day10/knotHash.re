...
let rec knot = (~lengths, ~numbers, ~position, ~skip) =>
  ...
  | [length, ...tail] =>
    let start = position;
    let end_ = position + length;
    let subArray = CircularArray.rev(CircularArray.sub(numbers, start, end_));
    let newNumbers = CircularArray.replace(numbers, start, subArray);
    /* Js.log(newNumbers); */
